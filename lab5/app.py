...
def create_app() -> Flask:
    ...
    # ВАЖНО: x_prefix=1 чтобы Flask корректно видел /lab5 из X-Forwarded-Prefix
    ...
    def _next_url_current_page():
        # Всегда добавляем префикс /lab5 для корректного next под nginx
        return "/lab5" + request.path
    ...
    def index():
        ...
        return render_template("users_list.html", users=users, me=user)
    ...
    def login():
        ...
        if request.method == "POST":
            ...
            if nxt and nxt.startswith("/") and not nxt.startswith("/lab5/"):
                nxt = "/lab5" + nxt
    ...
    def user_view(user_id: int):
        ...
        return render_template("user_view.html", user=user, me=get_current_user())
